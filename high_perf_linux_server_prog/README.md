...
- [序](#序)
- [TCP/IP协议族](#tcpip协议族)
    - [TCP/IP协议族体系结构以及主要协议](#tcpip协议族体系结构以及主要协议)
        - [数据链路层](#数据链路层)
        - [网络层](#网络层)
        - [传输层](#传输层)
        - [应用层](#应用层)
    - [封装(encapsulation)](#封装encapsulation)
        ...
        - [IP封装](#ip封装)
        - [数据链路层封装](#数据链路层封装)
    - [分用(demultiplexing)](#分用demultiplexing)
    - [测试网络](#测试网络)
    - [ARP协议工作原理](#arp协议工作原理)
        - [以太网ARP请求/应答报文详解](#以太网arp请求应答报文详解)
        - [ARP高速缓存的查看和修改](#arp高速缓存的查看和修改)
        - [使用tcpdump观察ARP通信过程](#使用tcpdump观察arp通信过程)
    - [DNS工作原理](#dns工作原理)
        - [Linux下访问DNS服务](#linux下访问dns服务)
        - [使用tcpdump观察DNS通信过程](#使用tcpdump观察dns通信过程)
    - [socket和TCP/IP协议族的关系](#socket和tcpip协议族的关系)
- [IP协议详解](#ip协议详解)
    - [IP服务的特点](#ip服务的特点)
    - [IPv4头部结构](#ipv4头部结构)
        - [使用tcpdump观察IPv4头部结构](#使用tcpdump观察ipv4头部结构)
    - [IP分片](#ip分片)
        - [使用tcpdump观察IP分片](#使用tcpdump观察ip分片)
    - [IP路由](#ip路由)
        - [IP模块工作流程](#ip模块工作流程)
        - [路由机制](#路由机制)
        - [路由表更新](#路由表更新)
...
- [TCP协议详解](#tcp协议详解)
    ...
    - [TCP头部结构](#tcp头部结构)
        - [TCP头部选项](#tcp头部选项)
        - [使用tcpdump观察TCP头部信息](#使用tcpdump观察tcp头部信息)
    - [TCP连接的建立和关闭](#tcp连接的建立和关闭)
        - [使用tcpdump观察TCP连接的建立和关闭](#使用tcpdump观察tcp连接的建立和关闭)
        - [半关闭状态](#半关闭状态)
    - [TCP状态转移](#tcp状态转移)
        - [TCP状态转移总图](#tcp状态转移总图)
    ...
    - [TCP超时重传](#tcp超时重传)
    - [拥塞控制](#拥塞控制)
- [TCP/IP通信案例：访问Internet上的Web服务器](#tcpip通信案例访问internet上的web服务器)
    - [HTTP通信](#http通信)
        - [HTTP请求](#http请求)
        - [HTTP应答](#http应答)
- [Linux网络编程基础API](#linux网络编程基础api)
    - [socket地址API](#socket地址api)
        - [主机字节序和网络字节序](#主机字节序和网络字节序)
        - [通用socket地址](#通用socket地址)
        - [专用socket地址](#专用socket地址)
        - [IP地址转换函数](#ip地址转换函数)
    - [创建socket](#创建socket)
    - [命名socket](#命名socket)
    - [监听socket](#监听socket)
    ...
    - [地址信息函数](#地址信息函数)
    - [socket选项](#socket选项)
        - [SO_REUSEADDR选项](#so_reuseaddr选项)
        - [SO_RCVBUF和SO_SNDBUF选项](#so_rcvbuf和so_sndbuf选项)
        - [SO_RCVLOWAT和SO_SNDLOWAT选项](#so_rcvlowat和so_sndlowat选项)
        - [SO_LINGER选项](#so_linger选项)
    - [网络信息API](#网络信息api)
        - [gethostbyname和gethostbyaddr](#gethostbyname和gethostbyaddr)
        - [getservbyname和getservbyport](#getservbyname和getservbyport)
        - [getaddrinfo](#getaddrinfo)
        - [gertnameinfo](#gertnameinfo)
- [高级I/O函数](#高级io函数)
    - [pipe函数](#pipe函数)
        - [双向管道](#双向管道)
    - [dup函数和dup2函数](#dup函数和dup2函数)
    - [readv函数和writev函数](#readv函数和writev函数)
    - [sendfile函数](#sendfile函数)
    - [mmap函数和munmap函数](#mmap函数和munmap函数)
    - [splice函数](#splice函数)
    - [tee函数](#tee函数)
    - [fcntl函数](#fcntl函数)
...
- [高性能服务器程序框架](#高性能服务器程序框架)
    ...
    - [两种高效的并发模式](#两种高效的并发模式)
        ...
        - [领导者/追随者模式](#领导者追随者模式)
- [I/O复用](#io复用-1)
    - [select系统调用](#select系统调用)
        - [select API](#select-api)
        - [文件描述符就绪条件](#文件描述符就绪条件)
        - [处理带外数据](#处理带外数据)
    - [poll系统调用](#poll系统调用)
    - [epoll系列系统调用](#epoll系列系统调用)
        - [内核事件表](#内核事件表)
        - [epoll_wait函数](#epoll_wait函数)
        - [EPOLLONESHOT事件](#epolloneshot事件)
    - [三组I/O复用函数的比较](#三组io复用函数的比较)
- [信号](#信号)
    - [Linux信号概述](#linux信号概述)
        - [发送信号](#发送信号)
        - [信号处理方式](#信号处理方式)
        ...
        - [中断系统调用](#中断系统调用)
    - [信号函数](#信号函数)
        - [signal系统调用](#signal系统调用)
        - [sigaction系统调用](#sigaction系统调用)
    - [信号集](#信号集)
        - [信号集函数](#信号集函数)
        - [进程信号掩码](#进程信号掩码)
        - [被挂起的信号](#被挂起的信号)
    ...
    - [网络编程相关信号](#网络编程相关信号)
        - [SIGURG](#sigurg)
- [定时器](#定时器)
    - [socket选项SO_RCVTIMEO和SO_SNDTIMEO](#socket选项so_rcvtimeo和so_sndtimeo)
    - [SIGALRM信号](#sigalrm信号)
        - [基于升序链表的定时器](#基于升序链表的定时器)
        - [处理非活动连接](#处理非活动连接)
    - [高性能定时器](#高性能定时器)
        - [时间轮](#时间轮)
        - [时间堆](#时间堆)
- [高性能I/O框架库Libevent](#高性能io框架库libevent)
    - [Libevent源码分析](#libevent源码分析)
- [多进程编程](#多进程编程)
    - [fork系统调用](#fork系统调用)
    - [exec系列调用](#exec系列调用)
    - [处理僵尸进程](#处理僵尸进程)
    ...
    - [信号量](#信号量)
        - [semget系统调用](#semget系统调用)
        - [semop系统调用](#semop系统调用)
        - [semctl系统调用](#semctl系统调用)
        - [特殊键值IPC_PRIVATE](#特殊键值ipc_private)
    - [共享内存](#共享内存)
        - [shmget系统调用](#shmget系统调用)
        - [shmat和shmdt系统调用](#shmat和shmdt系统调用)
        - [shmctl系统调用](#shmctl系统调用)
        - [共享内存的POSIX方法](#共享内存的posix方法)
    - [消息队列](#消息队列)
        - [msgget系统调用](#msgget系统调用)
        - [msgsnd系统调用](#msgsnd系统调用)
        - [msgrcv系统调用](#msgrcv系统调用)
        - [msgctl系统调用](#msgctl系统调用)
    - [在进程间传递文件描述符](#在进程间传递文件描述符)
- [多线程编程](#多线程编程)
    - [Linux线程概述](#linux线程概述)
        - [线程模型](#线程模型)
        - [Linux线程库](#linux线程库)
...
### 半同步/半异步模式
![concurrency_sync_async](./pictures/concurrency_sync_async.png)
* 同步(线程)
    * 程序完全按照代码序列的顺序执行
* 异步(线程)
    * 程序的执行需要由系统事件来驱动
...
![half_sync_half_async](./pictures/half_sync_half_async.png)

![half_sync_half_reactive](./pictures/half_sync_half_reactive.png)
* 异步线程只有一个，由主线程来充当，负责监听**所有socket**(包括监听socket和连接socket)上的事件
* 如果socket变为可读，表明有新的连接或数据的到来，主线程会将此socket插入请求队列
* 所有工作线程都睡眠在此请求队列上，当有任务到来时，通过竞争获得任务的接管权

半同步/半反应堆模式的缺点：
* 工作线程从请求队列中取出任务时，需要对请求队列加锁保护，浪费CPU时间
* 每个工作线程在同一时间只能处理一个客户端请求

![efficient_half_sync_half_reactive](./pictures/efficient_half_sync_half_reactive.png)
* 上图解决了半同步/半反应堆模式的缺点
* 主线程只管监听socket，连接socket由工作线程来管理
* 当有新的连接到来时，主线程就接受之并将新返回的连接socket派发给某个工作线程，此后该新socket上的任何I/O操作都由被选中的工作线程来处理，直到客户关闭连接

### 领导者/追随者模式
领导者/追随者模式时多个工作线程轮流获得事件源集合，轮流监听、分发并处理事件的一种模式。当领导者检测到I/O事件，首先要从线程池中推选出新的领导者线程，然后处理I/O事件。此时，新的领导者等待新的I/O事件，而原来的领导者处理I/O事件，二者实现了并发。
