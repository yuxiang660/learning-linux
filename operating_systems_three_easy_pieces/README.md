...
- [配置环境](#配置环境)
   - [安装“XV6”系统](#安装xv6系统)
- [第2章 操作系统介绍](#第2章-操作系统介绍)
   - [关键问题 - 如何将资源虚拟化？](#关键问题---如何将资源虚拟化)
   - [虚拟化CPU](#虚拟化cpu)
   - [虚拟化内存](#虚拟化内存)
   - [并发](#并发)
      - [关键问题 - 如何构建正确的并发程序？](#关键问题---如何构建正确的并发程序)
   - [持久性](#持久性)
      - [关键问题 - 如何持久地存储数据？](#关键问题---如何持久地存储数据)
   - [设计目标](#设计目标)
- [第4章 抽象：进程](#第4章-抽象进程)
   - [关键问题 - 如何提供有许多CPU的假象？](#关键问题---如何提供有许多cpu的假象)
   - [关键问题 - 操作系统以什么策略做出调度决定？](#关键问题---操作系统以什么策略做出调度决定)
   - [关键问题 -  什么是操作系统的策略和机制分离？](#关键问题----什么是操作系统的策略和机制分离)
   - [什么是进程？](#什么是进程)
   - [进程状态](#进程状态)
   - [进程数据结构](#进程数据结构)
- [第5章 插叙：进程API](#第5章-插叙进程api)
   - [关键问题 - 为什么设计`fork`和`exec`这种奇怪的接口来完成简单的、创建新进程的任务？](#关键问题---为什么设计fork和exec这种奇怪的接口来完成简单的创建新进程的任务)
- [第6章：受限直接执行](#第6章受限直接执行)
   - [关键问题 - 如何高效、可控地虚拟化CPU？](#关键问题---如何高效可控地虚拟化cpu)
   - [基本技巧：受限直接执行](#基本技巧受限直接执行)
      - [关键问题1: 受限制的操作 - 如何执行受限制的操作？](#关键问题1-受限制的操作---如何执行受限制的操作)
      - [关键问题2：在进程之间切换 - 如何重获CPU的控制权？](#关键问题2在进程之间切换---如何重获cpu的控制权)
      - [关键问题3：保存和恢复上下文](#关键问题3保存和恢复上下文)
- [第7章 进程调度：介绍](#第7章-进程调度介绍)
   - [关键问题 - 如何开发调度策略？](#关键问题---如何开发调度策略)
   - [调度指标](#调度指标)
   - [调度策略对比](#调度策略对比)
   - [结合I/O](#结合io)
- [第8章 调度：多级反馈队列 Multi-Level Feedback Queue, MLFQ](#第8章-调度多级反馈队列-multi-level-feedback-queue-mlfq)
   - [关键问题 - 没有完备的知识如何调度？](#关键问题---没有完备的知识如何调度)
   - [MLFQ 基本规则](#mlfq-基本规则)
   - [尝试1：如何改变优先级](#尝试1如何改变优先级)
   - [尝试2：提升优先级](#尝试2提升优先级)
   - [尝试3： 更好的计时方式](#尝试3-更好的计时方式)
   - [小结](#小结)
- [第9章 调度：比例份额](#第9章-调度比例份额)
   - [彩票调度](#彩票调度)
   - [步长调度](#步长调度)
   - [比例份额调度的应用](#比例份额调度的应用)
- [第10章 多处理器调度(高级)](#第10章-多处理器调度高级)
   - [关键问题 - 多处理器系统调度面临哪些问题？](#关键问题---多处理器系统调度面临哪些问题)
   - [背景：多处理器架构](#背景多处理器架构)
      - [缓存一致性(cache coherence)问题](#缓存一致性cache-coherence问题)
      - [缓存亲和度(cache affinity)问题](#缓存亲和度cache-affinity问题)
   - [单队列调度 SQMS, Single Queue Multiprocessor Scheduling](#单队列调度-sqms-single-queue-multiprocessor-scheduling)
   - [多队列调度 MQMS，Multi-Queue Multiprocessor Scheduling](#多队列调度-mqmsmulti-queue-multiprocessor-scheduling)
      - [关键问题 - 如何应对MQMS存在负载不均的问题？](#关键问题---如何应对mqms存在负载不均的问题)
      - [Linux多处理器调度](#linux多处理器调度)
- [第13章 抽象：地址空间](#第13章-抽象地址空间)
   - [关键问题 - 如何虚拟化内存？](#关键问题---如何虚拟化内存)
   - [虚拟内存系统的目标](#虚拟内存系统的目标)
- [第14章 插叙：内存操作API](#第14章-插叙内存操作api)
   - [关键问题 - 如何分配和管理内存？](#关键问题---如何分配和管理内存)
   - [内存类型](#内存类型)
   - [底层操作系统支持](#底层操作系统支持)
      - [系统调用brk](#系统调用brk)
      - [系统调用mmap](#系统调用mmap)
- [第15章 机制：地址转换](#第15章-机制地址转换)
   - [关键问题 - 如何高效、灵活地虚拟化内存](#关键问题---如何高效灵活地虚拟化内存)
   - [基于硬件的地址转换(hardware-based address translation)](#基于硬件的地址转换hardware-based-address-translation)
   - [一个例子](#一个例子)
   - [动态(基于硬件)重定位](#动态基于硬件重定位)
   - [重定位硬件支持：总结](#重定位硬件支持总结)
   - [操作系统的职责](#操作系统的职责)
   - [动态重定位的受限执行协议](#动态重定位的受限执行协议)
   - [小结](#小结-1)
- [第16章 分段](#第16章-分段)
   - [关键问题 - 怎么支持大地址空间？](#关键问题---怎么支持大地址空间)
   - [分段：泛化的基址/界限](#分段泛化的基址界限)
   - [我们引用哪个段](#我们引用哪个段)
      - [关键问题 - 硬件在地址转换时使用段寄存器。它如何知道段内的偏移量，以及地址引用了哪个段?](#关键问题---硬件在地址转换时使用段寄存器它如何知道段内的偏移量以及地址引用了哪个段)
      - [显式(explicit)方式](#显式explicit方式)
      - [隐式(implicit)方式](#隐式implicit方式)
   - [支持共享](#支持共享)
   - [细粒度与粗粒度的分段](#细粒度与粗粒度的分段)
   - [操作系统支持](#操作系统支持)
      - [外部碎片问题解决方案](#外部碎片问题解决方案)
   - [小结](#小结-2)
- [第17章 空闲空间管理](#第17章-空闲空间管理)
   - [关键问题 - 如何管理空闲空间？](#关键问题---如何管理空闲空间)
   - [底层机制](#底层机制)
      - [分割与合并](#分割与合并)
   - [嵌入空闲列表](#嵌入空闲列表)
   - [让堆增长](#让堆增长)
   - [基本策略](#基本策略)
      ...
      - [分离空闲列表](#分离空闲列表)
      - [伙伴系统](#伙伴系统)
- [第18章 分页：介绍](#第18章-分页介绍)
   - [关键问题 - 如何通过分页来实现虚拟内存？](#关键问题---如何通过分页来实现虚拟内存)
   - [一个简单例子](#一个简单例子)
      - [页表](#页表)
      - [页表存在哪里](#页表存在哪里)
      - [列表中究竟有什么](#列表中究竟有什么)
   ...
   - [内存追踪](#内存追踪)
   - [小结](#小结-3)
- [第19章 分页：快速地址转换 TLB(Translation Lookaside Buffer)](#第19章-分页快速地址转换-tlbtranslation-lookaside-buffer)
   - [关键问题 - 如何加速地址转换？](#关键问题---如何加速地址转换)
   - [硬件的帮助](#硬件的帮助)
   - [TLB的基本算法](#tlb的基本算法)
   - [示例：访问数组](#示例访问数组)
      - [尽可能利用缓存](#尽可能利用缓存)
   - [谁来处理TLB未命中](#谁来处理tlb未命中)
   - [上下文切换时对TLB的处理](#上下文切换时对tlb的处理)
      - [关键问题 - 进程切换时如何管理TLB的内容？](#关键问题---进程切换时如何管理tlb的内容)
   - [TLB替换策略](#tlb替换策略)
      - [关键问题 - 如何设计TLB替换策略？](#关键问题---如何设计tlb替换策略)
   - [实际系统的TLB表项](#实际系统的tlb表项)
   - [小结](#小结-4)
- [第20章 分页：较小的表](#第20章-分页较小的表)
   - [关键问题 - 如何让页表更小？](#关键问题---如何让页表更小)
   - [简单的解决方案：更大的页](#简单的解决方案更大的页)
   - [混合方法：分页和分段](#混合方法分页和分段)
   - [多级页表](#多级页表)
      - [详细的多级示例](#详细的多级示例)
      - [超过两级的页表](#超过两级的页表)
      - [地址转换过程：记住TLB](#地址转换过程记住tlb)
   - [将页表交换到磁盘](#将页表交换到磁盘)
   - [小结](#小结-5)
- [第21章 超越物理内存：机制](#第21章-超越物理内存机制)
   - [关键问题 - 如何超越物理内存？](#关键问题---如何超越物理内存)
   - [交换空间](#交换空间)
   - [存在位](#存在位)
   - [页错误](#页错误)
      - [关键问题 - 操作系统如何知道所需的页在哪里？](#关键问题---操作系统如何知道所需的页在哪里)
      - [为什么硬件不能处理页错误](#为什么硬件不能处理页错误)
   - [内存满了怎么办](#内存满了怎么办)
   - [页错误处理流程](#页错误处理流程)
      - [硬件在地址转换过程中所做的工作](#硬件在地址转换过程中所做的工作)
   - [操作系统在页错误时所做的工作](#操作系统在页错误时所做的工作)
   - [交换何时真正发生](#交换何时真正发生)
- [第22章 超越物理内存：策略](#第22章-超越物理内存策略)
   - [关键问题 - 如何决定提出哪个页？](#关键问题---如何决定提出哪个页)
   - [缓存管理](#缓存管理)
   - [最优替换策略](#最优替换策略)
   - [简单策略：FIFO和随机](#简单策略fifo和随机)
   - [利用历史数据：LRU Least-Recently-Used](#利用历史数据lru-least-recently-used)
   - [工作负载示例](#工作负载示例)
      - [无局部性工作负载](#无局部性工作负载)
      - [80-20工作负载](#80-20工作负载)
      - [循环顺序工作负载](#循环顺序工作负载)
   - [实现基于历史信息的算法](#实现基于历史信息的算法)
      - [关键问题 - 如何实现LRU替换策略？](#关键问题---如何实现lru替换策略)
   - [近似LRU](#近似lru)
   - [考虑脏页](#考虑脏页)
   - [其他虚拟内存策略](#其他虚拟内存策略)
   - [抖动](#抖动)
- [第23章 VAX/VMS 虚拟内存系统](#第23章-vaxvms-虚拟内存系统)
   - [背景](#背景)
      - [关键问题：如何避免通用性“魔咒”？](#关键问题如何避免通用性魔咒)
   - [一个真实的地址空间](#一个真实的地址空间)
   - [VAX页替换](#vax页替换)
      - [分段的FIFO替换策略](#分段的fifo替换策略)
   - [其他漂亮的虚拟内存技巧](#其他漂亮的虚拟内存技巧)
- [第26章 并发：介绍](#第26章-并发介绍)
   - [实例：线程创建](#实例线程创建)
   - [为什么更糟糕：共享数据](#为什么更糟糕共享数据)
   - [原子性愿望](#原子性愿望)
      - [关键并发术语](#关键并发术语)
   - [两个关键问题 - 同步原语和条件变量](#两个关键问题---同步原语和条件变量)
- [第27章 插叙：线程API](#第27章-插叙线程api)
   - [关键问题 - 如何创建和控制线程？](#关键问题---如何创建和控制线程)
   - [创建线程](#创建线程)
   - [线程完成](#线程完成)
   - [锁](#锁)
   - [条件变量](#条件变量)
...
- [第28章 锁](#第28章-锁)
   - [锁的基本思想](#锁的基本思想)
   - [Pthread锁](#pthread锁)
   - [关键问题 - 怎样实现一个锁？](#关键问题---怎样实现一个锁)
   - [评价锁](#评价锁)
   - [控制中断](#控制中断)
   - [测试并设置指令(原子交换)](#测试并设置指令原子交换)
      - [第一次锁实现：简单标志](#第一次锁实现简单标志)
   - [实现可用的自旋锁](#实现可用的自旋锁)
   - [比较并交换指令](#比较并交换指令)
   - [链接的加载和条件式存储指令](#链接的加载和条件式存储指令)
   - [获取并增加指令](#获取并增加指令)
   - [自旋过多：怎么办](#自旋过多怎么办)
   - [简单方法：让出来吧，宝贝](#简单方法让出来吧宝贝)
   - [使用队列：休眠代替自旋](#使用队列休眠代替自旋)
   - [不同操作系统，不同实现](#不同操作系统不同实现)
   - [两阶段锁](#两阶段锁)
- [第29章 基于锁的并发数据结构](#第29章-基于锁的并发数据结构)
   - [关键问题 - 如何给数据结构枷锁？](#关键问题---如何给数据结构枷锁)
   - [并发计数器](#并发计数器)
      - [简单的互斥](#简单的互斥)
      - [可扩展的计数 - 懒惰计数器](#可扩展的计数---懒惰计数器)
   - [并发链表](#并发链表)
   - [并发队列](#并发队列)
   - [并发散列表](#并发散列表)
- [第30章 条件变量](#第30章-条件变量)
   - [关键问题 - 如何等待一个条件？](#关键问题---如何等待一个条件)
   - [定义和程序](#定义和程序)
      - [是否需要状态变量done?](#是否需要状态变量done)
      - [如果线程在发信号和等待时都不加锁，会有什么问题？](#如果线程在发信号和等待时都不加锁会有什么问题)
      - [为什么对条件变量的wait用while而不是if？](#为什么对条件变量的wait用while而不是if)
   - [生产者/消费者(有界缓冲区)问题](#生产者消费者有界缓冲区问题)
      - [有问题的方案](#有问题的方案)
      - [较好但仍有问题的方案：使用while语句替代if](#较好但仍有问题的方案使用while语句替代if)
      - [单值缓冲区的生产者/消费者方案](#单值缓冲区的生产者消费者方案)
      - [最终的生产者/消费者方案](#最终的生产者消费者方案)
   - [覆盖条件](#覆盖条件)
   - [小结](#小结-7)
- [第31章 信号量](#第31章-信号量)
   - [关键问题 - 如何使用信号量？](#关键问题---如何使用信号量)
   - [信号量的定义](#信号量的定义)
   - [二值信号量(锁)](#二值信号量锁)
   - [信号量用作条件变量](#信号量用作条件变量)
   - [生产者/消费者(有界缓冲区)问题](#生产者消费者有界缓冲区问题-1)
   - [读者--写者锁](#读者--写者锁)
   - [哲学家就餐问题](#哲学家就餐问题)
      - [如何拿起或放下左右的餐具](#如何拿起或放下左右的餐具)
   - [如何实现信号量](#如何实现信号量)
   - [小心泛化](#小心泛化)
- [第32章 常见并发问题](#第32章-常见并发问题)
   - [有哪些类型的缺陷](#有哪些类型的缺陷)
   - [非死锁缺陷](#非死锁缺陷)
      - [违反原子性缺陷](#违反原子性缺陷)
      - [违反顺序缺陷](#违反顺序缺陷)
   - [死锁缺陷](#死锁缺陷)
      - [为什么发生死锁](#为什么发生死锁)
   - [小结](#小结-8)
- [第33章 基于事件的并发](#第33章-基于事件的并发)
   - [关键问题 - 不用线程，如何构建并发服务器？](#关键问题---不用线程如何构建并发服务器)
   - [基本想法：事件循环](#基本想法事件循环)
   - [重要API：select() 或 poll()](#重要apiselect-或-poll)
   - [使用select()](#使用select)
   - [阻塞系统调用](#阻塞系统调用)
   - [什么事情仍然很难](#什么事情仍然很难)
- [第36章 I/O设备](#第36章-io设备)
   - [关键问题 - 如何将I/O集成进计算机系统中？](#关键问题---如何将io集成进计算机系统中)
   - [系统架构](#系统架构)
   - [标准设备](#标准设备)
   - [标准协议](#标准协议)
      - [关键问题 - 如何减少轮询开销？](#关键问题---如何减少轮询开销)
   - [利用中断减少CPU开销](#利用中断减少cpu开销)
   - [利用DMA进行高效的数据传送](#利用dma进行高效的数据传送)
      - [关键问题 - 如何减少PIO的开销？](#关键问题---如何减少pio的开销)
   - [设备交互的方法](#设备交互的方法)
      - [关键问题 - 如何与设备通信？](#关键问题---如何与设备通信)
   - [纳入操作系统：设备驱动程序](#纳入操作系统设备驱动程序)
      - [关键问题 - 如何实现一个设备无关的操作系统？](#关键问题---如何实现一个设备无关的操作系统)
   - [案例研究：简单的IDE磁盘驱动程序](#案例研究简单的ide磁盘驱动程序)
      - [IDE的寄存器](#ide的寄存器)
      - [IDE的操作协议](#ide的操作协议)
   - [历史记录](#历史记录)
   - [小结](#小结-9)
- [第37章 磁盘驱动器](#第37章-磁盘驱动器)
   - [关键问题 - 如何存储和访问磁盘上的数据？](#关键问题---如何存储和访问磁盘上的数据)
   - [接口](#接口)
   - [简单的磁盘驱动器](#简单的磁盘驱动器)
      - [单磁道延迟：旋转延迟](#单磁道延迟旋转延迟)
      - [多磁道：寻道时间](#多磁道寻道时间)
   - [I/O时间：用数学](#io时间用数学)
   - [磁盘调度](#磁盘调度)
- [第38章 廉价冗余磁盘阵列(RAID)](#第38章-廉价冗余磁盘阵列raid)
   - [关键问题 - 如何得到大型、快速、可靠的磁盘？](#关键问题---如何得到大型快速可靠的磁盘)
   - [如何评估RAID](#如何评估raid)
- [第39章 插叙：文件和目录](#第39章-插叙文件和目录)
   - [关键问题 - 如何管理持久存储设备？](#关键问题---如何管理持久存储设备)
   - [文件和目录](#文件和目录)
   - [创建文件](#创建文件)
   - [读写文件](#读写文件)
   - [读取和写入，但不按顺序](#读取和写入但不按顺序)
   - [用fsync()立即写入](#用fsync立即写入)
   - [文件重命名](#文件重命名)
   - [获取文件信息](#获取文件信息)
   - [删除文件](#删除文件)
   - [创建目录](#创建目录)
   ...
   - [符号链接 - 软链接](#符号链接---软链接)
   - [创建并挂载文件系统](#创建并挂载文件系统)
      - [关键问题 - 如何从许多底层文件系统组建完整的目录树？](#关键问题---如何从许多底层文件系统组建完整的目录树)
- [第40章 文件系统实现](#第40章-文件系统实现)
   - [关键问题 - 如何实现简单的文件系统？](#关键问题---如何实现简单的文件系统)
   - [思考方式](#思考方式)
   - [整体组织](#整体组织)
   - [文件组织：index node(inode)](#文件组织index-nodeinode)
      - [多级索引](#多级索引)
   - [目录组织](#目录组织)
   - [空闲空间管理](#空闲空间管理)
   - [访问路径：读取和写入](#访问路径读取和写入)
      - [从磁盘读取文件](#从磁盘读取文件)
      - [写入磁盘](#写入磁盘)
   - [关键问题 - 如何降低文件系统I/O成本？](#关键问题---如何降低文件系统io成本)
   - [缓存和缓冲](#缓存和缓冲)
   - [小结](#小结-10)
- [第41章 局部性和快速文件系统FFS](#第41章-局部性和快速文件系统ffs)
   - [关键问题 - 如何组织磁盘数据以提高性能？](#关键问题---如何组织磁盘数据以提高性能)
   - [FFS：磁盘意识是解决方案](#ffs磁盘意识是解决方案)
   - [组织结构：柱面组](#组织结构柱面组)
   - [策略：如何分配文件和目录](#策略如何分配文件和目录)
      - [目录的放置](#目录的放置)
   - [大文件例外](#大文件例外)
   - [小文件的处理](#小文件的处理)
- [第42章 崩溃一致性：FSCK和日志](#第42章-崩溃一致性fsck和日志)
   - [关键问题 - 考虑到崩溃，如何更新磁盘？](#关键问题---考虑到崩溃如何更新磁盘)
   - [解决方案1：文件系统检查程序](#解决方案1文件系统检查程序)
   - [解决方案2：日志(或预写日志)](#解决方案2日志或预写日志)
   - [数据日志](#数据日志)
      - [恢复](#恢复)
      - [批处理日志更新](#批处理日志更新)
      - [更新文件的基本协议](#更新文件的基本协议)
      ...
      - [元数据日志](#元数据日志)
   - [小结](#小结-11)
- [第43章 日志结构文件系统 LFS](#第43章-日志结构文件系统-lfs)
   - [关键问题](#关键问题)
   - [按顺序写入磁盘](#按顺序写入磁盘)
   - [顺序而高效地写入](#顺序而高效地写入)
   - [从磁盘读取文件：回顾](#从磁盘读取文件回顾)
   - [一个新问题：垃圾回收](#一个新问题垃圾回收)
   - [确定块的死活](#确定块的死活)
   - [策略问题：要清理哪些块，何时清理](#策略问题要清理哪些块何时清理)
   - [LFS](#lfs)
- [第47章 分布式系统](#第47章-分布式系统)
   - [关键问题 - 如何构建在组件故障时仍能工作的系统？](#关键问题---如何构建在组件故障时仍能工作的系统)
   - [通信基础](#通信基础)
   - [不可靠的通信层](#不可靠的通信层)
   - [可靠的通信层](#可靠的通信层)
   - [通信抽象](#通信抽象)
   - [远程过程调用 RPC](#远程过程调用-rpc)
      - [存根生成器](#存根生成器)
      - [运行时库](#运行时库)
- [第48章 Sun的网络文件系统(NFS)](#第48章-sun的网络文件系统nfs)
   - [关键问题 - 如何构建分布式文件系统？](#关键问题---如何构建分布式文件系统)
   - [基本分布式文件系统](#基本分布式文件系统)
   - [快速崩溃恢复的关键：无状态](#快速崩溃恢复的关键无状态)
   - [NFSv2协议](#nfsv2协议)
      - [关键问题 - 如何定义无状态文件协议？](#关键问题---如何定义无状态文件协议)
   - [缓存一致性问题](#缓存一致性问题)
- [第49章 Andrew文件系统 AFS](#第49章-andrew文件系统-afs)
   - [小结](#小结-12)
...
# 第5章 插叙：进程API
## 关键问题 - 为什么设计`fork`和`exec`这种奇怪的接口来完成简单的、创建新进程的任务？
...
## 我们引用哪个段
### 关键问题 - 硬件在地址转换时使用段寄存器。它如何知道段内的偏移量，以及地址引用了哪个段?
