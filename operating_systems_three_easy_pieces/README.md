...
- [1. 配置环境](#1-配置环境)
   - [1.1. 安装“XV6”系统](#11-安装xv6系统)
- [2. 第2章 操作系统介绍](#2-第2章-操作系统介绍)
   - [2.1. 关键问题 - 如何将资源虚拟化？](#21-关键问题---如何将资源虚拟化)
   - [2.2. 虚拟化CPU](#22-虚拟化cpu)
   - [2.3. 虚拟化内存](#23-虚拟化内存)
   - [2.4. 并发](#24-并发)
      - [2.4.1. 关键问题 - 如何构建正确的并发程序？](#241-关键问题---如何构建正确的并发程序)
   - [2.5. 持久性](#25-持久性)
      - [2.5.1. 关键问题 - 如何持久地存储数据？](#251-关键问题---如何持久地存储数据)
   - [2.6. 设计目标](#26-设计目标)
- [3. 第4章 抽象：进程](#3-第4章-抽象进程)
   - [3.1. 关键问题 - 如何提供有许多CPU的假象？](#31-关键问题---如何提供有许多cpu的假象)
   - [3.2. 关键问题 - 操作系统以什么策略做出调度决定？](#32-关键问题---操作系统以什么策略做出调度决定)
   - [3.3. 关键问题 -  什么是操作系统的策略和机制分离？](#33-关键问题----什么是操作系统的策略和机制分离)
   - [3.4. 什么是进程？](#34-什么是进程)
   - [3.5. 进程状态](#35-进程状态)
   - [3.6. 进程数据结构](#36-进程数据结构)
- [4. 第5章 插叙：进程API](#4-第5章-插叙进程api)
   - [4.1. 关键问题 - 为什么设计`fork`和`exec`这种奇怪的接口来完成简单的、创建新进程的任务？](#41-关键问题---为什么设计fork和exec这种奇怪的接口来完成简单的创建新进程的任务)
- [5. 第6章：受限直接执行](#5-第6章受限直接执行)
   - [5.1. 关键问题 - 如何高效、可控地虚拟化CPU？](#51-关键问题---如何高效可控地虚拟化cpu)
   - [5.2. 基本技巧：受限直接执行](#52-基本技巧受限直接执行)
      - [5.2.1. 关键问题1: 受限制的操作 - 如何执行受限制的操作？](#521-关键问题1-受限制的操作---如何执行受限制的操作)
      - [5.2.2. 关键问题2：在进程之间切换 - 如何重获CPU的控制权？](#522-关键问题2在进程之间切换---如何重获cpu的控制权)
      - [5.2.3. 关键问题3：保存和恢复上下文](#523-关键问题3保存和恢复上下文)
- [6. 第7章 进程调度：介绍](#6-第7章-进程调度介绍)
   - [6.1. 关键问题 - 如何开发调度策略？](#61-关键问题---如何开发调度策略)
   - [6.2. 调度指标](#62-调度指标)
   - [6.3. 调度策略对比](#63-调度策略对比)
   - [6.4. 结合I/O](#64-结合io)
- [7. 第8章 调度：多级反馈队列 Multi-Level Feedback Queue, MLFQ](#7-第8章-调度多级反馈队列-multi-level-feedback-queue-mlfq)
   - [7.1. 关键问题 - 没有完备的知识如何调度？](#71-关键问题---没有完备的知识如何调度)
   - [7.2. MLFQ 基本规则](#72-mlfq-基本规则)
   - [7.3. 尝试1：如何改变优先级](#73-尝试1如何改变优先级)
   - [7.4. 尝试2：提升优先级](#74-尝试2提升优先级)
   - [7.5. 尝试3： 更好的计时方式](#75-尝试3-更好的计时方式)
   - [7.6. 小结](#76-小结)
- [8. 第9章 调度：比例份额](#8-第9章-调度比例份额)
   - [8.1. 彩票调度](#81-彩票调度)
   - [8.2. 步长调度](#82-步长调度)
   - [8.3. 比例份额调度的应用](#83-比例份额调度的应用)
- [9. 第10章 多处理器调度(高级)](#9-第10章-多处理器调度高级)
   - [9.1. 关键问题 - 多处理器系统调度面临哪些问题？](#91-关键问题---多处理器系统调度面临哪些问题)
   - [9.2. 背景：多处理器架构](#92-背景多处理器架构)
      - [9.2.1. 缓存一致性(cache coherence)问题](#921-缓存一致性cache-coherence问题)
      - [9.2.2. 缓存亲和度(cache affinity)问题](#922-缓存亲和度cache-affinity问题)
   - [9.3. 单队列调度 SQMS, Single Queue Multiprocessor Scheduling](#93-单队列调度-sqms-single-queue-multiprocessor-scheduling)
   - [9.4. 多队列调度 MQMS，Multi-Queue Multiprocessor Scheduling](#94-多队列调度-mqmsmulti-queue-multiprocessor-scheduling)
      - [9.4.1. 关键问题 - 如何应对MQMS存在负载不均的问题？](#941-关键问题---如何应对mqms存在负载不均的问题)
      - [9.4.2. Linux多处理器调度](#942-linux多处理器调度)
- [10. 第13章 抽象：地址空间](#10-第13章-抽象地址空间)
   - [10.1. 关键问题 - 如何虚拟化内存？](#101-关键问题---如何虚拟化内存)
   - [10.2. 虚拟内存系统的目标](#102-虚拟内存系统的目标)
- [11. 第14章 插叙：内存操作API](#11-第14章-插叙内存操作api)
   - [11.1. 关键问题 - 如何分配和管理内存？](#111-关键问题---如何分配和管理内存)
   - [11.2. 内存类型](#112-内存类型)
   - [11.3. 底层操作系统支持](#113-底层操作系统支持)
      - [11.3.1. 系统调用brk](#1131-系统调用brk)
      - [11.3.2. 系统调用mmap](#1132-系统调用mmap)
- [12. 第15章 机制：地址转换](#12-第15章-机制地址转换)
   - [12.1. 关键问题 - 如何高效、灵活地虚拟化内存](#121-关键问题---如何高效灵活地虚拟化内存)
   - [12.2. 基于硬件的地址转换(hardware-based address translation)](#122-基于硬件的地址转换hardware-based-address-translation)
   - [12.3. 一个例子](#123-一个例子)
   - [12.4. 动态(基于硬件)重定位](#124-动态基于硬件重定位)
   - [12.5. 重定位硬件支持：总结](#125-重定位硬件支持总结)
   - [12.6. 操作系统的职责](#126-操作系统的职责)
   - [12.7. 动态重定位的受限执行协议](#127-动态重定位的受限执行协议)
   - [12.8. 小结](#128-小结)
- [13. 第16章 分段](#13-第16章-分段)
   - [13.1. 关键问题 - 怎么支持大地址空间？](#131-关键问题---怎么支持大地址空间)
   - [13.2. 分段：泛化的基址/界限](#132-分段泛化的基址界限)
   - [13.3. 我们引用哪个段](#133-我们引用哪个段)
      - [13.3.1. 关键问题 - 硬件在地址转换时使用段寄存器。它如何知道段内的偏移量，以及地址引用了哪个段?](#1331-关键问题---硬件在地址转换时使用段寄存器它如何知道段内的偏移量以及地址引用了哪个段)
      - [13.3.2. 显式(explicit)方式](#1332-显式explicit方式)
      - [13.3.3. 隐式(implicit)方式](#1333-隐式implicit方式)
   - [13.4. 支持共享](#134-支持共享)
   - [13.5. 细粒度与粗粒度的分段](#135-细粒度与粗粒度的分段)
   - [13.6. 操作系统支持](#136-操作系统支持)
      - [13.6.1. 外部碎片问题解决方案](#1361-外部碎片问题解决方案)
   - [13.7. 小结](#137-小结)
- [14. 第17章 空闲空间管理](#14-第17章-空闲空间管理)
   - [14.1. 关键问题 - 如何管理空闲空间？](#141-关键问题---如何管理空闲空间)
   - [14.2. 底层机制](#142-底层机制)
      - [14.2.1. 分割与合并](#1421-分割与合并)
   - [14.3. 嵌入空闲列表](#143-嵌入空闲列表)
   - [14.4. 让堆增长](#144-让堆增长)
   - [14.5. 基本策略](#145-基本策略)
      - [14.5.1. 最优匹配](#1451-最优匹配)
      - [14.5.2. 最差匹配](#1452-最差匹配)
      - [14.5.3. 首次匹配](#1453-首次匹配)
      - [14.5.4. 下次匹配](#1454-下次匹配)
      - [14.5.5. 分离空闲列表](#1455-分离空闲列表)
      - [14.5.6. 伙伴系统](#1456-伙伴系统)
- [15. 第18章 分页：介绍](#15-第18章-分页介绍)
   - [15.1. 关键问题 - 如何通过分页来实现虚拟内存？](#151-关键问题---如何通过分页来实现虚拟内存)
   - [15.2. 一个简单例子](#152-一个简单例子)
      - [15.2.1. 页表](#1521-页表)
      - [15.2.2. 页表存在哪里](#1522-页表存在哪里)
      - [15.2.3. 列表中究竟有什么](#1523-列表中究竟有什么)
   - [15.3. 分页：也很慢](#153-分页也很慢)
   - [15.4. 内存追踪](#154-内存追踪)
   - [15.5. 小结](#155-小结)
- [16. 第19章 分页：快速地址转换 TLB(Translation Lookaside Buffer)](#16-第19章-分页快速地址转换-tlbtranslation-lookaside-buffer)
   - [16.1. 关键问题 - 如何加速地址转换？](#161-关键问题---如何加速地址转换)
   - [16.2. 硬件的帮助](#162-硬件的帮助)
   - [16.3. TLB的基本算法](#163-tlb的基本算法)
   - [16.4. 示例：访问数组](#164-示例访问数组)
      - [16.4.1. 尽可能利用缓存](#1641-尽可能利用缓存)
   - [16.5. 谁来处理TLB未命中](#165-谁来处理tlb未命中)
   - [16.6. 上下文切换时对TLB的处理](#166-上下文切换时对tlb的处理)
      - [16.6.1. 关键问题 - 进程切换时如何管理TLB的内容？](#1661-关键问题---进程切换时如何管理tlb的内容)
   - [16.7. TLB替换策略](#167-tlb替换策略)
      - [16.7.1. 关键问题 - 如何设计TLB替换策略？](#1671-关键问题---如何设计tlb替换策略)
   - [16.8. 实际系统的TLB表项](#168-实际系统的tlb表项)
   - [16.9. 小结](#169-小结)
- [17. 第20章 分页：较小的表](#17-第20章-分页较小的表)
   - [17.1. 关键问题 - 如何让页表更小？](#171-关键问题---如何让页表更小)
   - [17.2. 简单的解决方案：更大的页](#172-简单的解决方案更大的页)
   - [17.3. 混合方法：分页和分段](#173-混合方法分页和分段)
   - [17.4. 多级页表](#174-多级页表)
      - [17.4.1. 详细的多级示例](#1741-详细的多级示例)
      - [17.4.2. 超过两级的页表](#1742-超过两级的页表)
      - [17.4.3. 地址转换过程：记住TLB](#1743-地址转换过程记住tlb)
   - [17.5. 将页表交换到磁盘](#175-将页表交换到磁盘)
   - [17.6. 小结](#176-小结)
- [18. 第21章 超越物理内存：机制](#18-第21章-超越物理内存机制)
   - [18.1. 关键问题 - 如何超越物理内存？](#181-关键问题---如何超越物理内存)
   - [18.2. 交换空间](#182-交换空间)
   - [18.3. 存在位](#183-存在位)
   - [18.4. 页错误](#184-页错误)
      - [18.4.1. 关键问题 - 操作系统如何知道所需的页在哪里？](#1841-关键问题---操作系统如何知道所需的页在哪里)
      - [18.4.2. 为什么硬件不能处理页错误](#1842-为什么硬件不能处理页错误)
   - [18.5. 内存满了怎么办](#185-内存满了怎么办)
   - [18.6. 页错误处理流程](#186-页错误处理流程)
      - [18.6.1. 硬件在地址转换过程中所做的工作](#1861-硬件在地址转换过程中所做的工作)
   - [18.7. 操作系统在页错误时所做的工作](#187-操作系统在页错误时所做的工作)
   - [18.8. 交换何时真正发生](#188-交换何时真正发生)
- [19. 第22章 超越物理内存：策略](#19-第22章-超越物理内存策略)
   - [19.1. 关键问题 - 如何决定提出哪个页？](#191-关键问题---如何决定提出哪个页)
   - [19.2. 缓存管理](#192-缓存管理)
   - [19.3. 最优替换策略](#193-最优替换策略)
   - [19.4. 简单策略：FIFO和随机](#194-简单策略fifo和随机)
   - [19.5. 利用历史数据：LRU Least-Recently-Used](#195-利用历史数据lru-least-recently-used)
   - [19.6. 工作负载示例](#196-工作负载示例)
      - [19.6.1. 无局部性工作负载](#1961-无局部性工作负载)
      - [19.6.2. -20工作负载](#1962--20工作负载)
      - [19.6.3. 循环顺序工作负载](#1963-循环顺序工作负载)
   - [19.7. 实现基于历史信息的算法](#197-实现基于历史信息的算法)
      - [19.7.1. 关键问题 - 如何实现LRU替换策略？](#1971-关键问题---如何实现lru替换策略)
   - [19.8. 近似LRU](#198-近似lru)
   - [19.9. 考虑脏页](#199-考虑脏页)
   - [19.10. 其他虚拟内存策略](#1910-其他虚拟内存策略)
   - [19.11. 抖动](#1911-抖动)
- [20. 第23章 VAX/VMS 虚拟内存系统](#20-第23章-vaxvms-虚拟内存系统)
   - [20.1. 背景](#201-背景)
      - [20.1.1. 关键问题：如何避免通用性“魔咒”？](#2011-关键问题如何避免通用性魔咒)
   - [20.2. 一个真实的地址空间](#202-一个真实的地址空间)
   - [20.3. VAX页替换](#203-vax页替换)
      - [20.3.1. 分段的FIFO替换策略](#2031-分段的fifo替换策略)
   - [20.4. 其他漂亮的虚拟内存技巧](#204-其他漂亮的虚拟内存技巧)
- [21. 第26章 并发：介绍](#21-第26章-并发介绍)
   - [21.1. 实例：线程创建](#211-实例线程创建)
   - [21.2. 为什么更糟糕：共享数据](#212-为什么更糟糕共享数据)
   - [21.3. 原子性愿望](#213-原子性愿望)
      - [21.3.1. 关键并发术语](#2131-关键并发术语)
   - [21.4. 两个关键问题 - 同步原语和条件变量](#214-两个关键问题---同步原语和条件变量)
- [22. 第27章 插叙：线程API](#22-第27章-插叙线程api)
   - [22.1. 关键问题 - 如何创建和控制线程？](#221-关键问题---如何创建和控制线程)
   - [22.2. 创建线程](#222-创建线程)
   - [22.3. 线程完成](#223-线程完成)
   - [22.4. 锁](#224-锁)
   - [22.5. 条件变量](#225-条件变量)
   - [22.6. 小结](#226-小结)
- [23. 第28章 锁](#23-第28章-锁)
   - [23.1. 锁的基本思想](#231-锁的基本思想)
   - [23.2. Pthread锁](#232-pthread锁)
   - [23.3. 关键问题 - 怎样实现一个锁？](#233-关键问题---怎样实现一个锁)
   - [23.4. 评价锁](#234-评价锁)
   - [23.5. 控制中断](#235-控制中断)
   - [23.6. 测试并设置指令(原子交换)](#236-测试并设置指令原子交换)
      - [23.6.1. 第一次锁实现：简单标志](#2361-第一次锁实现简单标志)
   - [23.7. 实现可用的自旋锁](#237-实现可用的自旋锁)
   - [23.8. 比较并交换指令](#238-比较并交换指令)
   - [23.9. 链接的加载和条件式存储指令](#239-链接的加载和条件式存储指令)
   - [23.10. 获取并增加指令](#2310-获取并增加指令)
   - [23.11. 自旋过多：怎么办](#2311-自旋过多怎么办)
   - [23.12. 简单方法：让出来吧，宝贝](#2312-简单方法让出来吧宝贝)
   - [23.13. 使用队列：休眠代替自旋](#2313-使用队列休眠代替自旋)
   - [23.14. 不同操作系统，不同实现](#2314-不同操作系统不同实现)
   - [23.15. 两阶段锁](#2315-两阶段锁)
- [24. 第29章 基于锁的并发数据结构](#24-第29章-基于锁的并发数据结构)
   - [24.1. 关键问题 - 如何给数据结构枷锁？](#241-关键问题---如何给数据结构枷锁)
   - [24.2. 并发计数器](#242-并发计数器)
      - [24.2.1. 简单的互斥](#2421-简单的互斥)
      - [24.2.2. 可扩展的计数 - 懒惰计数器](#2422-可扩展的计数---懒惰计数器)
   - [24.3. 并发链表](#243-并发链表)
   - [24.4. 并发队列](#244-并发队列)
   - [24.5. 并发散列表](#245-并发散列表)
- [25. 第30章 条件变量](#25-第30章-条件变量)
   - [25.1. 关键问题 - 如何等待一个条件？](#251-关键问题---如何等待一个条件)
   - [25.2. 定义和程序](#252-定义和程序)
      - [25.2.1. 是否需要状态变量done?](#2521-是否需要状态变量done)
      - [25.2.2. 如果线程在发信号和等待时都不加锁，会有什么问题？](#2522-如果线程在发信号和等待时都不加锁会有什么问题)
      - [25.2.3. 为什么对条件变量的wait用while而不是if？](#2523-为什么对条件变量的wait用while而不是if)
   - [25.3. 生产者/消费者(有界缓冲区)问题](#253-生产者消费者有界缓冲区问题)
      - [25.3.1. 有问题的方案](#2531-有问题的方案)
      - [25.3.2. 较好但仍有问题的方案：使用while语句替代if](#2532-较好但仍有问题的方案使用while语句替代if)
      - [25.3.3. 单值缓冲区的生产者/消费者方案](#2533-单值缓冲区的生产者消费者方案)
      - [25.3.4. 最终的生产者/消费者方案](#2534-最终的生产者消费者方案)
   - [25.4. 覆盖条件](#254-覆盖条件)
   - [25.5. 小结](#255-小结)
- [26. 第31章 信号量](#26-第31章-信号量)
   - [26.1. 关键问题 - 如何使用信号量？](#261-关键问题---如何使用信号量)
   - [26.2. 信号量的定义](#262-信号量的定义)
   - [26.3. 二值信号量(锁)](#263-二值信号量锁)
   - [26.4. 信号量用作条件变量](#264-信号量用作条件变量)
   - [26.5. 生产者/消费者(有界缓冲区)问题](#265-生产者消费者有界缓冲区问题)
   - [26.6. 读者--写者锁](#266-读者--写者锁)
   - [26.7. 哲学家就餐问题](#267-哲学家就餐问题)
      - [26.7.1. 如何拿起或放下左右的餐具](#2671-如何拿起或放下左右的餐具)
   - [26.8. 如何实现信号量](#268-如何实现信号量)
   - [26.9. 小心泛化](#269-小心泛化)
- [27. 第32章 常见并发问题](#27-第32章-常见并发问题)
   - [27.1. 有哪些类型的缺陷](#271-有哪些类型的缺陷)
   - [27.2. 非死锁缺陷](#272-非死锁缺陷)
      - [27.2.1. 违反原子性缺陷](#2721-违反原子性缺陷)
      - [27.2.2. 违反顺序缺陷](#2722-违反顺序缺陷)
   - [27.3. 死锁缺陷](#273-死锁缺陷)
      - [27.3.1. 为什么发生死锁](#2731-为什么发生死锁)
   - [27.4. 小结](#274-小结)
- [28. 第33章 基于事件的并发](#28-第33章-基于事件的并发)
   - [28.1. 关键问题 - 不用线程，如何构建并发服务器？](#281-关键问题---不用线程如何构建并发服务器)
   - [28.2. 基本想法：事件循环](#282-基本想法事件循环)
   - [28.3. 重要API：select() 或 poll()](#283-重要apiselect-或-poll)
   - [28.4. 使用select()](#284-使用select)
   - [28.5. 阻塞系统调用](#285-阻塞系统调用)
   - [28.6. 什么事情仍然很难](#286-什么事情仍然很难)
- [29. 第36章 I/O设备](#29-第36章-io设备)
   - [29.1. 关键问题 - 如何将I/O集成进计算机系统中？](#291-关键问题---如何将io集成进计算机系统中)
   - [29.2. 系统架构](#292-系统架构)
   - [29.3. 标准设备](#293-标准设备)
   - [29.4. 标准协议](#294-标准协议)
      - [29.4.1. 关键问题 - 如何减少轮询开销？](#2941-关键问题---如何减少轮询开销)
   - [29.5. 利用中断减少CPU开销](#295-利用中断减少cpu开销)
   - [29.6. 利用DMA进行高效的数据传送](#296-利用dma进行高效的数据传送)
      - [29.6.1. 关键问题 - 如何减少PIO的开销？](#2961-关键问题---如何减少pio的开销)
   - [29.7. 设备交互的方法](#297-设备交互的方法)
      - [29.7.1. 关键问题 - 如何与设备通信？](#2971-关键问题---如何与设备通信)
   - [29.8. 纳入操作系统：设备驱动程序](#298-纳入操作系统设备驱动程序)
      - [29.8.1. 关键问题 - 如何实现一个设备无关的操作系统？](#2981-关键问题---如何实现一个设备无关的操作系统)
   - [29.9. 案例研究：简单的IDE磁盘驱动程序](#299-案例研究简单的ide磁盘驱动程序)
      - [29.9.1. IDE的寄存器](#2991-ide的寄存器)
      - [29.9.2. IDE的操作协议](#2992-ide的操作协议)
   - [29.10. 历史记录](#2910-历史记录)
   - [29.11. 小结](#2911-小结)
- [30. 第37章 磁盘驱动器](#30-第37章-磁盘驱动器)
   - [30.1. 关键问题 - 如何存储和访问磁盘上的数据？](#301-关键问题---如何存储和访问磁盘上的数据)
   - [30.2. 接口](#302-接口)
   - [30.3. 简单的磁盘驱动器](#303-简单的磁盘驱动器)
      - [30.3.1. 单磁道延迟：旋转延迟](#3031-单磁道延迟旋转延迟)
      - [30.3.2. 多磁道：寻道时间](#3032-多磁道寻道时间)
   - [30.4. I/O时间：用数学](#304-io时间用数学)
   - [30.5. 磁盘调度](#305-磁盘调度)
- [31. 第38章 廉价冗余磁盘阵列(RAID)](#31-第38章-廉价冗余磁盘阵列raid)
   - [31.1. 关键问题 - 如何得到大型、快速、可靠的磁盘？](#311-关键问题---如何得到大型快速可靠的磁盘)
   - [31.2. 如何评估RAID](#312-如何评估raid)
- [32. 第39章 插叙：文件和目录](#32-第39章-插叙文件和目录)
   - [32.1. 关键问题 - 如何管理持久存储设备？](#321-关键问题---如何管理持久存储设备)
   - [32.2. 文件和目录](#322-文件和目录)
   - [32.3. 创建文件](#323-创建文件)
   - [32.4. 读写文件](#324-读写文件)
   - [32.5. 读取和写入，但不按顺序](#325-读取和写入但不按顺序)
   - [32.6. 用fsync()立即写入](#326-用fsync立即写入)
   - [32.7. 文件重命名](#327-文件重命名)
   - [32.8. 获取文件信息](#328-获取文件信息)
   - [32.9. 删除文件](#329-删除文件)
   - [32.10. 创建目录](#3210-创建目录)
   - [32.11. 读取目录](#3211-读取目录)
   - [32.12. 删除目录](#3212-删除目录)
   - [32.13. 硬链接](#3213-硬链接)
   - [32.14. 符号链接 - 软链接](#3214-符号链接---软链接)
   - [32.15. 创建并挂载文件系统](#3215-创建并挂载文件系统)
      - [32.15.1. 关键问题 - 如何从许多底层文件系统组建完整的目录树？](#32151-关键问题---如何从许多底层文件系统组建完整的目录树)
- [33. 第40章 文件系统实现](#33-第40章-文件系统实现)
   - [33.1. 关键问题 - 如何实现简单的文件系统？](#331-关键问题---如何实现简单的文件系统)
   - [33.2. 思考方式](#332-思考方式)
   - [33.3. 整体组织](#333-整体组织)
   - [33.4. 文件组织：index node(inode)](#334-文件组织index-nodeinode)
      - [33.4.1. 多级索引](#3341-多级索引)
   - [33.5. 目录组织](#335-目录组织)
   - [33.6. 空闲空间管理](#336-空闲空间管理)
   - [33.7. 访问路径：读取和写入](#337-访问路径读取和写入)
      - [33.7.1. 从磁盘读取文件](#3371-从磁盘读取文件)
      - [33.7.2. 写入磁盘](#3372-写入磁盘)
   - [33.8. 关键问题 - 如何降低文件系统I/O成本？](#338-关键问题---如何降低文件系统io成本)
   - [33.9. 缓存和缓冲](#339-缓存和缓冲)
   - [33.10. 小结](#3310-小结)
- [34. 第41章 局部性和快速文件系统FFS](#34-第41章-局部性和快速文件系统ffs)
   - [34.1. 关键问题 - 如何组织磁盘数据以提高性能？](#341-关键问题---如何组织磁盘数据以提高性能)
   - [34.2. FFS：磁盘意识是解决方案](#342-ffs磁盘意识是解决方案)
   - [34.3. 组织结构：柱面组](#343-组织结构柱面组)
   - [34.4. 策略：如何分配文件和目录](#344-策略如何分配文件和目录)
      - [34.4.1. 目录的放置](#3441-目录的放置)
   - [34.5. 大文件例外](#345-大文件例外)
   - [34.6. 小文件的处理](#346-小文件的处理)
- [35. 第42章 崩溃一致性：FSCK和日志](#35-第42章-崩溃一致性fsck和日志)
   - [35.1. 关键问题 - 考虑到崩溃，如何更新磁盘？](#351-关键问题---考虑到崩溃如何更新磁盘)
   - [35.2. 解决方案1：文件系统检查程序](#352-解决方案1文件系统检查程序)
   - [35.3. 解决方案2：日志(或预写日志)](#353-解决方案2日志或预写日志)
   - [35.4. 数据日志](#354-数据日志)
      - [35.4.1. 恢复](#3541-恢复)
      - [35.4.2. 批处理日志更新](#3542-批处理日志更新)
      - [35.4.3. 更新文件的基本协议](#3543-更新文件的基本协议)
      - [35.4.4. 使日志有限](#3544-使日志有限)
      - [35.4.5. 元数据日志](#3545-元数据日志)
   - [35.5. 小结](#355-小结)
- [36. 第43章 日志结构文件系统 LFS](#36-第43章-日志结构文件系统-lfs)
   - [36.1. 关键问题](#361-关键问题)
   - [36.2. 按顺序写入磁盘](#362-按顺序写入磁盘)
   - [36.3. 顺序而高效地写入](#363-顺序而高效地写入)
   - [36.4. 从磁盘读取文件：回顾](#364-从磁盘读取文件回顾)
   - [36.5. 一个新问题：垃圾回收](#365-一个新问题垃圾回收)
   - [36.6. 确定块的死活](#366-确定块的死活)
   - [36.7. 策略问题：要清理哪些块，何时清理](#367-策略问题要清理哪些块何时清理)
   - [36.8. LFS](#368-lfs)
- [37. 第47章 分布式系统](#37-第47章-分布式系统)
   - [37.1. 关键问题 - 如何构建在组件故障时仍能工作的系统？](#371-关键问题---如何构建在组件故障时仍能工作的系统)
   - [37.2. 通信基础](#372-通信基础)
   - [37.3. 不可靠的通信层](#373-不可靠的通信层)
   - [37.4. 可靠的通信层](#374-可靠的通信层)
   - [37.5. 通信抽象](#375-通信抽象)
   - [37.6. 远程过程调用 RPC](#376-远程过程调用-rpc)
      - [37.6.1. 存根生成器](#3761-存根生成器)
      - [37.6.2. 运行时库](#3762-运行时库)
- [38. 第48章 Sun的网络文件系统(NFS)](#38-第48章-sun的网络文件系统nfs)
   - [38.1. 关键问题 - 如何构建分布式文件系统？](#381-关键问题---如何构建分布式文件系统)
   - [38.2. 基本分布式文件系统](#382-基本分布式文件系统)
   - [38.3. 快速崩溃恢复的关键：无状态](#383-快速崩溃恢复的关键无状态)
   - [38.4. NFSv2协议](#384-nfsv2协议)
      - [38.4.1. 关键问题 - 如何定义无状态文件协议？](#3841-关键问题---如何定义无状态文件协议)
   - [38.5. 缓存一致性问题](#385-缓存一致性问题)
- [39. 第49章 Andrew文件系统 AFS](#39-第49章-andrew文件系统-afs)
   - [39.1. 小结](#391-小结)
...
# 3. 第4章 抽象：进程
## 3.1. 关键问题 - 如何提供有许多CPU的假象？
* 时分共享
   * CPU在时间上划分给多个任务
* 空分共享
   * 磁盘在空间上划分给多个任务

## 3.2. 关键问题 - 操作系统以什么策略做出调度决定？
* 历史信息
   * 哪个程序在最后一分钟要运行更多？
* 工作负载
   * 运行什么类型的程序？
...
# 4. 第5章 插叙：进程API
## 4.1. 关键问题 - 为什么设计`fork`和`exec`这种奇怪的接口来完成简单的、创建新进程的任务？
...
Limited Direct Execution (LDE)协议有两个阶段：
...
### 5.2.2. 关键问题2：在进程之间切换 - 如何重获CPU的控制权？
...
# 7. 第8章 调度：多级反馈队列 Multi-Level Feedback Queue, MLFQ
多级反馈队列解决了：
* 在不知道工作要运行多久时，无法使用SJF或STCF优化周转时间
* 优化响应时间，但不能像轮转那样牺牲了周转时间

## 7.1. 关键问题 - 没有完备的知识如何调度？
...
### 9.2.1. 缓存一致性(cache coherence)问题
...
## 9.3. 单队列调度 SQMS, Single Queue Multiprocessor Scheduling
...
* 缓存亲和性
   ...
   * 即使做了如下改进，E任务还是没法保证缓存亲和性: <br>
   ![sqms](./pictures/sqms_2.png)

## 9.4. 多队列调度 MQMS，Multi-Queue Multiprocessor Scheduling
...
# 12. 第15章 机制：地址转换
## 12.1. 关键问题 - 如何高效、灵活地虚拟化内存
* 如何实现高效的内存虚拟化？
* 如何提供应用程序所需的灵活性？
* 如何保持控制应用程序可访问的内存位置，从而确保应用程序的内存访问受到合理的限制？
* 如何高效得实现这一切？

## 12.2. 基于硬件的地址转换(hardware-based address translation)
...
## 13.3. 我们引用哪个段
### 13.3.1. 关键问题 - 硬件在地址转换时使用段寄存器。它如何知道段内的偏移量，以及地址引用了哪个段?

### 13.3.2. 显式(explicit)方式
...
# 16. 第19章 分页：快速地址转换 TLB(Translation Lookaside Buffer)
## 16.1. 关键问题 - 如何加速地址转换？
...
## 16.7. TLB替换策略
### 16.7.1. 关键问题 - 如何设计TLB替换策略？
在向TLB添加新项时，应该替换哪个旧项？目标当然是减小TLB未命中率，从而改进性能。几种典型的策略：
...
## 19.5. 利用历史数据：LRU Least-Recently-Used
...
### 25.2.2. 如果线程在发信号和等待时都不加锁，会有什么问题？
```c
void thr_exit() {
   done = 1;
   pthread_cond_signal(&c);
...
### 25.2.3. 为什么对条件变量的wait用while而不是if？
...
### 25.3.2. 较好但仍有问题的方案：使用while语句替代if
```c
void* producer(void* arg) {
   int i;
   int loops = (intptr_t)arg;
...
### 29.9.2. IDE的操作协议
从上文中提到的操作系统与设备的交互协议可知，一般操作设备的包括：等待设备就绪 -> 向设备写入数据与命令 -> 等待设备执行完成 -> 检测错误。
...
# 31. 第38章 廉价冗余磁盘阵列(RAID)
## 31.1. 关键问题 - 如何得到大型、快速、可靠的磁盘？
...
## 32.15. 创建并挂载文件系统
### 32.15.1. 关键问题 - 如何从许多底层文件系统组建完整的目录树？
...
# 35. 第42章 崩溃一致性：FSCK和日志
## 35.1. 关键问题 - 考虑到崩溃，如何更新磁盘？
...
# 38. 第48章 Sun的网络文件系统(NFS)

## 38.1. 关键问题 - 如何构建分布式文件系统？
* 如何构建分布式文件系统？
* 要考虑哪些关键方面？
* 哪里容易出错？
* 我们可以从现有系统中学到什么？
