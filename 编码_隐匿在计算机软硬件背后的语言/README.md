...
# 第22章 操作系统
## CP/M系统
* 如何将存在磁盘上的CP/M系统加载到内存中？<br>
只读存储器(ROM)存放一小段称为引导程序(bootstrap loader, 操作系统的其余部分可以通过这段代码的自举操作被高效地引导)。当开机时，磁盘上最开始的128字节的扇区内容会首先由引导程序加载到内存并运行，这个扇区包含有特定的代码，可以把CP/M中的其余部分加载到内存中，整个过程称为操作系统的引导(booting)。
* 操作系统引导完成后，RAM的最高地址区域用来存放CP/M，加载完后，内存组织结构如下：
![os_ram](./pictures/os_ram.png)
   * 控制台命令处理程序(Console Command Processor, CPP)
      * 在命令提示符后面敲入命令并按回车键，控制台命令处理程序会执行该命令，然后将执行的结果显示到屏幕上
      * 执行完命令后，命令提示符又会显示在屏幕上，等待下一次输入
      * 如果输入的是CP/M不能识别的命令，CP/M会默认为输入的是保存在磁盘上的一个程序名。控制台命令处理程序负责在磁盘上查找此文件，如果找到，此文件会被CP/M从磁盘加载到临时程序区域，该区域的地址从0100h开始，一旦文件被调入内存即可运行。
   * 基本磁盘操作系统(Basic Disk Operating System, BDOS)
      * 包含许多小程序，CP/M的每一项功能都可由它们完成
      * 主要功能是维护磁盘上的文件系统
      * 文件系统经常要与终端设备打交道，所以BDOS经常要调用CP/M基本输入/输出系统BIOS中的一些子程序
   * 基本输入/输出系统(Basic Input/Output System, BIOS)
      * 可以对硬件进行访问，比如键盘、显示器等
      * 是唯一需要了解计算机硬件的程序
   * 临时程序区(Transient Program Area, TPA)
      * 58KB, 而且一开始是空的