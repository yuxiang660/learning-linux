...
## 概述
### 仿真和验证
* 下面着重放在Verilog语言本身的仿真上，至于门级网表和布线结果的功能(时序)仿真，以及硬件加速、逻辑分析仪等验证手段，参考其他文献

### 什么是Testbench
![testbench](./pictures/testbench.png)
* 用途
   * Testbench是用来产生测试激励给待验证设计(DUV)，或者称为待测设计(DUT)，同时检测DUV的输出是否与预期一致，达到验证设计功能的目的
* 主要功能
   * 为DUT提供激励信号
   * 正确实例化DUT
   * 将仿真数据显示在终端或者存为文件，也可以显示在波形窗口中以供分析检查
   * 复杂设计可以使用EDA工具，实现结果的自动检查

## 建立Testbench，仿真设计
![testbench_mpi](./pictures/testbench_mpi.png)

### 编写仿真激励
* 仿真激励与被测对象的连接
   * 在Testbench中实例化DUV，DUV的端口与Testbench中的信号互连
* 使用initial语句和always语句
   * 主动产生激励时用initial
   * 被动地检测相应时使用always
* 时钟、复位的写法
* 产生值序列
   * 可采用阻塞赋值的方法来产生一系列的值变化
* 利用系统函数和系统任务
   * $display
   * $setup, $hold
* 从文本文件中读出和写入数据
...
* 事件类型
   * 更新事件
      * 线网或寄存器值的任何改变被认为是一个更新事件(Update Event)
   * 计算事件
      * 当一个更新事件被执行后，所有对该事件敏感的进程都将以随机顺序计算，称为计算事件
* 事件之间的关系
   * 更新事件会导致计算事件发生，计算事件也会导致更新事件发生

### 进程
* Verilog的独立执行单元，由一个一个进程组成，包括
   * 原语(Primitives)
   * 模块(Modules)
   * Initial过程块
   * Always过程块
...
## 分层事件队列于仿真参考模型
### 分层事件队列
* Verilog事件队列分为5个不同的区域<br>
   ![sim_queue_layer](./pictures/sim_queue_layer.png)
   * 事件时按照一定的规则被加入到5个区域中任意一个区域的，但是只从其中“活跃事件”区域出队，出队之后该事件将会立即执行
* 仿真器先按照仿真时间对事件进行排序，然后再在当前仿真时间里按照事件的优先级顺序进行排序
   * 活跃事件优先级最高
      * 在活跃事件之间执行顺序是随机的，阻塞赋值，连续赋值以及非阻塞赋值的右式计算的执行是随机的
   * 当仿真器前进到某个时刻后，该时刻的所有事件也会被分类到当前仿真时间事件队列中

## 时序模型与延时

### 仿真模型
* 数字硬件的模型称为仿真模型，如always语句块
...
* 分类
   * 门级时序模型
      * 对任何变化敏感
   * 过程时序模型
      * initial和always语句支队输入的一个子集敏感

### 在Verilog语言中增加延时
* 电路中的两种延时
   * 惯性延时，也称惰性延时
      * 小于这个延时的输入变化都不会对输出造成影响
   * 传到延时
* 在阻塞赋值语句中增加延时(不推荐)
   ```verilog
   always @(a or b)
   ...
   always @(a or b)
      sum = #5 a + b; // 不推荐
   ```
   * 因为这样会同时延时计算和更新这两个事件，而无法模拟惯性延时和传导延时
* 在非阻塞赋值中增加延时(只在右式中有意义)
   ```verilog
   always @(a or b)
      #5 sum <= a + b; // 不推荐
...
## Verilog中的几种赋值语句
* [参考](https://www.cnblogs.com/nanoty/archive/2012/10/21/2733017.html)
* 分类
   * 连续赋值(和过程块平级)
      * `assign`开始
   * 过程赋值(出现在过程块中)
...
### 本质
* 语义本质
   * 阻塞赋值是在本语句中“右式计算”和“左式更新”完全完成之后，才开始执行下一条语句
   * 非阻塞赋值的右式计算术语活跃事件，计算事件执行完后，产生更新事件放入队列，不马上执行。当完成其他当前仿真事件的活跃事件和非活跃事件之后，才会执行当前事件的非阻塞赋值更新事件

* 电路本质
   * 非阻塞赋值<br>
      ![nonblock_assign](./pictures/nonblock_assign.png)
      ```verilog
      ...
      ```
   * 阻塞赋值<br>
      ![block_assign](./pictures/block_assign.png)
      ```verilog
      always @(posedge clk) begin
         q1 = d;
         q2 = q1;
