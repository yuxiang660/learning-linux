...
*  代码例子
   * 带循环的算法：![calc_ll](./code/calc_ll/main.c)
   * 不带循环的LL(1)递归下降：![calc_ll_1](./code/calc_ll_1/main.c)

### 表格驱动的自上而下语法分析
表驱动的LL语法分析是递归下降分析器结构的翻版。不是通过过程调用链去追溯出一个树遍历，而是维护一个堆栈，在其中保存着它期望在将来能看到的符号列表。驱动程序在一个循环中不断迭代：
* 如果堆栈顶是一个终结符，分析器就试着用这个终结符去与来自扫描器的单词匹配
   * 如果不匹配，就启动错误恢复
* 如果栈顶是一个非终结符，分析器就用这个非终结符和下一个可用单词，一并作为一个二维表格的下表，该表格给出应该预测哪个产生式
