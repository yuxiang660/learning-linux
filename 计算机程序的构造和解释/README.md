...
## 编译
如何在一部控制器不是Scheme解释器的寄存器机器上运行Scheme程序？

通用计算机的控制器也是一个寄存器机器语言的解释器，该语言与我们前面看到的东西类似，这样的一个语言被称为这台计算机的本机语言，或称为机器语言。用这种机器语言写出的程序就是指令的序列，它们使用这部机器的数据通路。

为了在高级语言和寄存器机器语言之间的鸿沟上架设起一座桥梁，由两种策略：
* “解释”策略
    * 用有关机器的本机语言写出一个解释器，它设法配置好这部机器，使它能够执行某个语言的程序。被解释的程序用一个数据结构表示。解释器遍历这种数据结构，分析源程序的情况。在这样的过程中，需要调用取自库的适当的基本子程序，以模拟源程序所需要的行为。
    * 为程序开发和排除错误提供了一个更强大的环境，因为被执行的源代码在运行期间都是可用的
* “编译”策略
    * 将源程序翻译为用这部机器的本机语言写出的等价程序(目标程序)，可大大提升程序的执行效率


编译器优于解释器的地方在于：
* 解释器在每次求值一个表达式时，都需要去做对这个表达式的分类工作，需要检查表达式的表是否结束
* 采用编译器的情况下，编译器可用考察它所处理的表达式，在产生的代码里避免所有并不必要的堆栈操作

解释器对于交互式的程序开发和排错式非常好的，因为程序执行的各个步骤都以这些抽象的方式组织起来了，因此更容易被程序员理解。编译后的代码执行得更快，因为程序执行步骤在机器语言层面上，编译器也可以自由地去做各种跨越高层抽象的优化。
...
* 目标和连接
    * compile有两个参数
        * 目标target
        * 连接描述符linkage
            * 描述相关表达式的编译结果代码在完成自己的执行之后，应该如何继续下去
                * 继续序列里的下一条指令(采用连接描述符next)
                * 从被编译的过程返回(采用连接描述符return)
                * 跳到下一个命名的入口点(指定标号)

### 编译代码的实例
我们通过形式调用compile，编译其中递归定义的factorial过程的定义：
```c
(compile
    `(define (factorial n)
        (if (= n 1)
...
```
* define表达式的值应该放入寄存器val
* 调用compile-definition去编译出计算被赋的值的代码(以val为目标)
* 随后安装这一定义的代码
* 随后将这一define的值放入目标寄存器的代码
* 再后面式最后的连接代码
