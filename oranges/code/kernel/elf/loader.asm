...
InitKernel:
        xor   esi, esi
        mov   cx, word [BaseOfKernelFilePhyAddr+2Ch];`. ecx <- pELFHdr->e_phnum, program header table中条目的个数，即段的个数
        movzx ecx, cx                               ;/
...
.Begin:
        ...
        cmp   eax, 0                      ; PT_NULL
        jz    .NoAction
        push  dword [esi + 010h]  	;size ; p_filesz在program header中的偏移是0x10，表示段在文件中的长度
        mov   eax, [esi + 04h]            ; p_offset在program header中的偏移是0x04，表示段的第一个字节在文件中的偏移
        add   eax, BaseOfKernelFilePhyAddr;
        push  eax									; src
        push  dword [esi + 08h]     		; dst，p_vaddr在program header中的偏移是0x08，表示段的第一个字节在内存中的虚拟地址
        call  MemCpy                      ; memcpy((void*)(pPHdr->p_vaddr),uchCode + pPHdr->p_offset,pPHdr->p_filesz）;
        add   esp, 12                     ; 将3个参数出栈
.NoAction:
