...
## 如何进入32位保护模式？
代码可参见例子["pmode_mbr"](./code/protected_mode_mbr/mbr.asm)，其通过配置GDT，再跳转到32位保护模式。

进入32位保护模式：
* 目的
   * 初始化GDT，再进入32保护模式，再直接向端口写入字符，显示在屏幕上
* 过程
   * 读取GDT初始地址，并初始化GDT，配置不同的段描述符
   * 配置GDTR，并转到32位保护模式(CR0的Protection Enable,PE位)
   * 进入32位模式后，初始化段选择器，其会根据GDT配置段选择子和高速缓存器
   * 段选择器配置完成后，就可以直接操作32位寄存器和内存了，每次访问都会访问保护

## 如何在保护模式下加载一个用户程序？
代码可参见例子["kernel"](./code/kernel)。

保护模式下加载用户程序的实现：
* 目的
   * 用一个简单的内核程序在保护模式下加载用户程序，由于内核大小可能会大于512字节，所以通过一个单独的引导程序，从实模式切换到保护模式，再加载内核，然后内核再加载用户程序
* 引导程序[mbr](./code/kernel/mbr.asm)
   * 在GDT安装引导程序的段描述符
      * 0~4G线性空间全局数据段(和内核共用)
      * 引导代码段
      * 堆栈段(和内核共用)
      * 字符显示段(和内核共用)
   * 从实模式切换到保护模式
   * 加载内核到内存指定位置`0x00040000`
   * 根据内核头，在GDT中安装内核程序的段描述符
      * 内核公共例程段
      * 内核代码段
      * 内核数据段
   * 将权限转交给内核程序的入口函数
* 内核程序[core](./code/kernel/core.asm)
   * 读取用户程序头得到用户程序大小
   * 在用户内存空间(起始于`ram_alloc:0x00100000`)开辟指定大小，并从硬盘往此处加载用户程序
   * 根据用户程序头，在GDT中安装用户程序的段描述符，并修改值对应的选择子到用户程序对应位置(内核程序写死了，所以没有此步骤)
      * 用户程序头部段
      * 用户程序代码段
      * 用户程序数据段
      * 用户程序堆栈段
         * 内核需要根据用户程序指定的堆栈大小，在用户空间中分配堆栈空间，同时设置和是的段描述符
   * 保护内核堆栈指针，由于用户程序有自己的堆栈指针
   * 将权限转交给用户程序的入口函数
* 用户程序[mbr](./code/kernel/prog.asm)
   * 根据已经配置好的选择子，配置对应的段寄存器
   * 执行相关操作
   * 结束后返回权限给内核
...
# 程序的动态加载和执行
## 关键问题
所有的段在使用之前，都必须以描述符的形式在描述符表中进行定义，那么像操作系统这样的软件，又怎么能够加载和执行其他各种用户程序呢？

## 保护模式下的用户程序加载内存分布
![mem_kernel](./pictures/mem_kernel.png)

* GDT最大可达到64KB
* 内核程序大小不固定，从0x00040000开始，最大可到0x0009FFFF。从0x000A0000往上，是ROM BIOS硬件专有的
* 0x000B8000往上32KB，是文本模式的显示缓冲区
* 最后从1MB开始的大量空间都是留给用户程序用的

## 内核的结构、功能和加载
内核分为四部分：
* 初始化代码，主引导程序[“mbr”](./code/kernel/mbr.asm)
   * 用于从BIOS那里接管处理器和计算机硬件的控制权
   * 安装最基本的段描述符<br>
   ![des_base](./pictures/des_base.png)
      * 进入保护模式后，要继续执行主引导程序的后半部分代码，所以要创建初始代码段
      * 堆栈段在进入内核后，又会变成内核的堆栈
   * 初始化最初的执行环境
   * 从硬盘读取和加载内核的剩余部分
   * 安装内核的段描述符到GDT：`setup`<br>
   ![gdt_kernel](./pictures/gdt_kernel.png)
      * 重新从标号pdgt处取得GDT的基地址，为其添加描述符，并修改它的大小，然后用lgdt指令重新加载一遍GDTR寄存器
* 内核代码段
   * 利用`cpuid`指令加载CPU信息
   * 加载和重定位用户程序：`load_relocate_program`
      * 现代操作系统的内存管理要记住所有可以分配的内存，将它们分成块，当要求分配内存时，内存管理程序将查找并分配那些大小相符的空闲块；还要负责回收用户终止执行的内存；当内存不足时，还有将很少被访问的内存移动到硬盘
* 内核数据段
* 内核例程段

# 任务和特权级保护
![load_whole](./pictures/load_whole.png)
## 局部描述符表LDT(Local Descriptor Table)
![ldt](./pictures/ldt.png)

LDT是针对每个任务(用户进程)，和GDT一样，最大也是64KB，可存8192个描述符。

## 任务状态段(Task State Segment)
![tss](./pictures/tss.png)

在切换任务过程中需要保存状态。任务状态段(Task State Segment: TSS)就是用于保存任务状态信息的。任务状态段TSS具有固定的格式，最小尺寸是104字节，处理器在任务切换的时候读取其中的信息。

## 全局空间和局部空间
![global_local_space](./pictures/global_local_space.png)

全局部分是所有任务共有的：
   * 含有操作系统的软件和库程序，以及可以调用的系统服务和数据
   * 由全局描述符GDT来定义
私有部分是每个任务私有的：
   * 每个任务各自的数据和代码
   * 由局部描述符LDT来定义
...
依从
* 描述符的TYPE字段有一个C位
   * C=0，这样的代码只能供同特权级的程序使用
   * C=1，这样的代码段称为以从代码段，可以从特权级比它低的程序调用并进入，注意不改变当前特权级CPL，段寄存器CS的CPL字段不发生变化

门描述符
* 调用门(Call Gate)<br>
![call_gate_des](./pictures/call_gate_des.png)
   * 如果调用门描述符的DPL为2，那么只有特权级为0、1和2的程序才允许使用该调用门，特权级为3的程序使用此门将引发处理器异常中断
   * 不同特权级之间的过程调用<br>
   ![call_gate_rules](./pictures/call_gate_rules.png)
      * `jmp far`指令
         * 不改变当前特权级别
      * `call far`指令
         * 当前特权级别会提升到目标代码段的特权级别
* 中断门/陷阱门
   * 作为中断处理过程使用
* 任务门
   * 用来执行任务切换
...
特权级检查总结
* 非依从
   * 将控制直接转移到非以从的代码段，要求当前特权级CPL和请求特权级RPL都等于目标代码段描述符的DPL
* 依从
   * 要求当前特权级CPL和请求特权级RPL都低于，或者和目标代码描述符的DPL相同
* 特权级别高的程序可以访问低特权级别的数据段，但低特权级别的程序不能访问高特权级别的数据段
* 任何时候，栈段的特权级别必须和当前特权级CPL相同

## 如何加载用户程序并创建任务
按处理器的要求标准，要使一个程序成为“任务”，并且能够参与任务切换和调度，那不是简简单单就能行的，必须要有LDT和TSS。
* 自定义TCB(task control block)用于记录每个任务所需要的信息，不是处理器要求，而是为了方便程序实现<br>
![tcb](./pictures/tcb.png)
   * 内核为每个任务开辟了TCB空间，通过链表管理，没增加一个任务，就在链表尾部添加一个TCB<br>
   ![tcb](./pictures/tcb_link.png)
* `load_relocate_program` - 加载并重定位用户程序
   * 需要传入两个参数<br>
   ![load_relocate_program_stack](./pictures/load_relocate_program_stack.png)
      * 用户程序的起始逻辑扇区号
      * 以及它的任务控制块TCB线性地址
   * 为每个任务分配LDT
   ...
   * 创建局部描述符表
      ...
      * 在LDT中安装程序堆栈段
* 创建0、1和2特权级的栈<br>
任务在运行时，需要调用内核或者操作系统的例程。这可以认为是从同一个任务的局部地址空间转移到全局地址空间工作。而且，在这个过程中涉及特权级的变化，需要通过调用门。通过调用门的控制转移通常会改变当前特权级CPL，同时还要切换到与目标代码段特权级相同的栈。
   * 分别为0、1和2创建内存空间作为各自的栈，并登记到TCB中
* 在GDT中登录LDT描述符
* 创建内存TSS空间
   * 在TSS空间中登记各特权栈的“特权级堆栈初始ESP”和“特权级堆栈段选择子”
   * 登记任务的I/O位图偏移
* 在GDT中登录TSS描述符

## 如何从任务的0特权级全局空间转移到它自己的3特权级空间正常执行？
* 使TR和LDTR寄存器指向这个任务
* 假装从调用门返回
