         ...
         ;创建1#描述符，存储器段，这是一个数据段，对应0~4GB的线性地址空间
         mov dword [ebx+0x08],0x0000ffff    ;基地址为0，段界限为0xfffff
         mov dword [ebx+0x0c],0x00cf9200    ;粒度为4KB，存储器段描述符 

         ;创建2#描述符，代码段，创建保护模式下初始代码段描述符
         mov dword [ebx+0x10],0x7c0001ff    ;基地址为0x00007c00，512字节 
         mov dword [ebx+0x14],0x00409800    ;粒度为1个字节，代码段描述符 

         ;创建3#描述符，数据段，创建以上代码段的别名描述符
         mov dword [ebx+0x18],0x7c0001ff    ;基地址为0x00007c00，512字节
         mov dword [ebx+0x1c],0x00409200    ;粒度为1个字节，数据段描述符
         ;创建4#描述符，堆栈段
         mov dword [ebx+0x20],0x7c00fffe    ;基地址为0x00007c00，界限=FFFFE
         mov dword [ebx+0x24],0x00cf9600    ;粒度为4KB，堆栈段
         ...
         [bits 32]                          
  flush:                                     
         mov eax,0x0018                     ;3#描述符，数据段
         mov ds,eax
      
         mov eax,0x0008                     ;1#描述符，存储器段，加载数据段(0..4GB)选择子，
         mov es,eax
         mov fs,eax
         mov gs,eax
      
         mov eax,0x0020                     ;4#描述符，堆栈段，0000 0000 0010 0000；
         mov ss,eax
         xor esp,esp                        ;ESP <- 0
