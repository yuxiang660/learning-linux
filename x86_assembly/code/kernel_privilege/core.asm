...
load_relocate_program:                      ;加载并重定位用户程序
  ...
  .b1:
         ...
         ;安装头部段描述符到LDT中 
         mov ebx,esi                        ;TCB的基地址
         call fill_descriptor_in_ldt        ;把刚才创建的描述符安装到LDT中
...
start:
  ...
  .b3:
         push ecx   
         mov eax,[edi+256]                  ;该条目入口点的32位偏移地址，前256字节是例程的名字，后6字节是例程的地址(前4字节是例程在目标代码段内的偏移量，后2字节是例程所在代码段的选择子)
         ...
         ;对门进行测试 
         mov ebx,message_2
         call far [salt_1+256]              ;通过门显示信息(偏移量将被忽略)，处理器在执行这条指令时，会用该选择子访问GDT/LDT，检查那个选择子，看它指向的时调用门描述符，还是普通的代码段描述符。如果时前者，就按调用门来处理；如果时后者，还按一般的段间控制转移处理
                                            ;通过调用门试试控制转移时，处理器只用选择子部分，32位偏移量部分被丢弃
         mov ebx,message_3                    
         call sys_routine_seg_sel:put_string ;在内核中调用例程不需要通过门
      
         ;创建任务控制块。这不是处理器的要求，而是我们自己为了方便而设立的
         mov ecx,0x46                       ;TCB结构需要0x46字节的内存空间
         call sys_routine_seg_sel:allocate_memory
         ...
         ltr [ecx+0x18]                     ;加载任务状态段，加载任务寄存器TR 
         lldt [ecx+0x10]                    ;加载LDT，加载局部描述符表寄存器LDTR
