         ...
         ;以下常量定义部分。内核的大部分内容都应当固定，它们在GDT中的描述符由引导程序加载
...
allocate_memory:                            ;分配内存
         ...
         mov ds,eax                         ;为了从ram_alloc中拿出分配内存的起始地址，配置DS
...
load_relocate_program:                      ;加载并重定位用户程序
  ...
  .b1:
         ...
         ;建立程序头部段描述符，head_len会被更改为该段在GDT中的选择子
         pop edi                            ;恢复程序装载的首地址 
         ...
         mov [edi+0x04],cx                  ;将该段的选择子写回到用户程序头部，即重定位

         ;建立程序代码段描述符，code_seg会被更改为该段在GDT中的选择子
         mov eax,edi
         ...
         ;建立程序数据段描述符，data_seg会被更改为该段在GDT中的选择子
         mov eax,edi
         ...
         ;建立程序堆栈段描述符，stack_seg会被更改为该段在GDT中的选择子
         mov ecx,[edi+0x0c]                 ;4KB的倍率，用户程序建议的堆栈大小
         mov ebx,0x000fffff
         sub ebx,ecx                        ;得到段界限
         mov eax,4096                        
         mul dword [edi+0x0c]               ;用4096(4KB)乘以用户建议的堆栈大小，得到实际所需堆栈大小
  ...
  .b3:
         ...
         mov ecx,64                         ;检索表中，每条目的比较次数 256/4 = 64
         repe cmpsd                         ;每次比pi'pepipe
         jnz .b4
         mov eax,[esi]                      ;若匹配，esi恰好指向其后的地址数据，，找到匹配后，将6字节传到用户程序
...
start:
         ...
         mov [esp_pointer],esp               ;临时保存堆栈指针，保存内核的堆栈指针，用户用户程序返回时，从这个内存位置还原内核堆栈指针
