         ...
         core_base_address equ 0x00040000   ;常数，内核加载的起始内存地址，EQU不占内存，内核目标内存位置
         core_start_sector equ 0x00000001   ;常数，内核的起始逻辑扇区号，内核源硬盘起始位置
  ...
  flush:                                  
         ...
         mov eax,core_start_sector          ;内核磁盘源地址
         mov ebx,edi                        ;起始地址，内核目标内存地址
         call read_hard_disk_0              ;以下读取程序的起始部分（一个扇区） 
      
         ;以下判断整个程序有多大
         mov eax,[edi]                      ;核心程序尺寸，从内核头中读取内核长度
         xor edx,edx 
 ...
 setup:
         mov esi,[0x7c00+pgdt+0x02]         ;不可以在代码段内寻址pgdt，因为代码段受写保护，但可以
                                            ;通过4GB的段来访问，得到GDT的基地址，DS中的线性基地址是0x00000000
         ;建立公用例程段描述符，此时edi指向了加载入内存的起始地址，0x04偏移在内核头中是系统公用例程段
         mov eax,[edi+0x04]                 ;公用例程代码段起始汇编地址
         mov ebx,[edi+0x08]                 ;核心数据段汇编地址
         sub ebx,eax
         dec ebx                            ;公用例程段界限 
         add eax,edi                        ;公用例程段基地址，公共例程段的起始汇编地址(eax)+内核的加载地址(edi)
         mov ecx,0x00409800                 ;字节粒度的代码段描述符
         call make_gdt_descriptor
         mov [esi+0x28],eax                 ;跟在原来的GDT后面，添加一个8字节公用例程段描述符
         mov [esi+0x2c],edx
         ...
         mov word [0x7c00+pgdt],63          ;描述符表的界限，又添加了3个，现在总共8个描述符
                                        
         lgdt [0x7c00+pgdt]                  

         jmp far [edi+0x10]                 ;内核代码的核心代码段入口点是0x10
