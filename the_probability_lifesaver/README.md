...
## 条件概率
* 在已知某个事件的前提下，我们考察的焦点会集中在样本空间的一个子集上；这类概率被称为"条件概率"，记作Pr(A|B)，读作“已知B时A的概率”

## 三门问题
* 三门问题的前提是：
   * 主持人知道那扇门后面是汽车
   * 主持人开门后，是问选手是否换门，而不是让选手再选一次
* 解释1：互斥原理
   * 选择换门赢的概率 = 选手第一次选错的概率：2/3
   * 选择不换门赢的概率 = 选手第一次就选对的概率：1/3
* 解释2：贝叶斯定理
   * 定义：事件A在事件B发生的条件下发生的概率，和事件B在事件A发生的条件下发生的概率是不同的，贝叶斯定理确定了这两者之间的关系
   * 设，事件A=选手不换门选对，事件B=主持人开了山羊的门
   * P(A|B)
      * P(A|B)表示在主持人开了山羊的门后，选手不换门选对的概率
      * 那么(1-P(A|B))表示在在主持人开了山羊的门后，选手换门赢的概率。
   * P(B|A)
      * 表示在选手不换门的并选对时，主持人开了山羊门的概率
         * 如果主持人事先知道门后面的情况，那么P(B|A)=1
         * 否则，由于选手已经选对，主持人肯定只能开山羊的门，因此：P(B|A)=1
   * P(A)
      * 表示选手不换选对的概率，P(A)=1/3
   * P(B)
      * 表示主持人开了山羊门的概率
         * 如果主持人事先知道门后面的情况，那么P(B)=1
         * 否则，P(B)=2/3
   * P(A|B) = P(B|A)*P(A)/P(B)
      * 如果主持人事先知道门后面的情况，那么P(A|B) = 1 * (1/3) / 1 = 1/3。因此选手选择换门赢的概率是：2/3
      * 如果主持人事先不知道门后面的情况，那么P(A|B) = 1 * (1/3) / (2/3) = 1/2。因此选手选择换门赢的概率是：1/2

## 贝叶斯定理
* 假阳、假阴问题
   * 问题：感染率为1/15000，假阳率1%，假阴率0.1%，已知检测为阳性，求感染的概率
      * 思路：
         * 事件A=感染，事件B=检测阳性，P(假阳)=P(B|非A)，P(假阴)=P(非B|A)，P(感染率)=P(A)
         * P(A|B) = P(B|A)*P(A)/P(B)
            * P(B) = P(B|A) * P(A) + P(B|非A) * P(非A) = 99.9% * P(A) + 1% * P(非A)
         * P(感染|阳性) = P(阳性|感染) * P(感染) / P(阳性)
...
* 样本空间S的一个划分{A1, A2, ...}满足
   ...
   * 所有Ai的并 = S
* 关键问题
   * 我们会经常遇到这样的情况：很想知道事件A发生的概率，但是只得到了A和其他某些事件Bi同时发生的概率
   * 已知一堆人中，P(流感∩养狗)=0.08，P(流感∩无养狗)=0.07，求P(流感)
      * P(流感) = P(流感|养狗) * P(养狗) + P(流感|无养狗) * P(无养狗) = P(流感∩养狗) + P(流感∩无养狗)
      * 利用期望计数解法更直观，假设有100人，8人即得流感又养狗，7人得流感但没有养狗，因此一种15人得了流感
* 新生儿存活率
   * P(剖腹产)=30%，P(死亡率|剖腹产)=0.2%，P(存活率|非剖腹产)=99.1%，求P(存活率)
      * P(存活率) = P(存活率|剖腹产) * P(剖腹产) + P(存活率|非剖腹产) * P(非剖腹产)
