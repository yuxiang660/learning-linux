...
## 阶乘和二项式问题
### 循环次序
* 问题1：5人围坐一桌，有多少种不同的相对坐法
   * 固定一个人，然后就是排序问题：4！
* 问题2(哲学家进餐问题)：n人围坐，左右各一根筷子
   * 约束1：同时拿到筷子才能吃东西(计算机中资源和进程的问题)
   * 约束2：未拿到两根筷子时，不会放下手中的筷子
   * 约束3：如果筷子被拿走，必须等待
      * 那么，如果每个人都拿左边的筷子，那么都吃不上
## 容斥方法
* 容斥方法的目的：通过交求并，因为大部分情况下交的概率更好求。

## 排错
* 问题：n个信封不同地址，对应n封信，所有信都没有放入正确信封的方法有多少种?
* 思路：等同问题，至少有一封信放对的方法有多少种？再通过容斥方法，转换成交的概率。
* 解答：Dn / n! = 1 - 1/1! + 1/2! - 1/3! + ... + (-1)^n/n! = e^-1 ≈ 36.79%
...
* 离散型随机变量X就是定义在一个离散的结果空间Ω(这意味着Ω是有限的活至多可数的)上的实值函数
   * 数学描述，ω∈Ω指定了一个实数X(ω)
   * 随机变量要应用于结果空间的元素上

## 离散型随机变量：概率密度函数
* 概率密度函数 VS 概率函数
   * 概率函数是概率空间三元组中的概念，定义在样本空间的子集上
   * 而概率密度函数是概率函数在随机变量上的体现，定义在随机变量上
   * 设X是一个随机变量，定义在离散结果空间Ω上，那么X的概率密度函数就是X取某个特定值的概率：
      * fX(x) = Prob(ω∈Ω：X(ω)=x)

## 离散型随机变量：累积分布函数
* 设X是一个随机变量，它定义在一个有限的活至多可数的离散结果空间Ω上。X的概率密度函数(fX)是X取某个特定值的概率。累计分布函数(FX)表示X不超过某个特定值的概率。
   * fX(x) = Prob(ω∈Ω：X(ω)=x)
   * FX(x) = Prob(ω∈Ω：X(ω)≤x)
...
# 卷积和变量替换
## 定义和性质
* 卷积的出现是为了便于计算随机变量之和的概率密度函数
   * ![folding](./pictures/folding.png)

# 特殊分布

# 离散分布
## 伯努利分布
* 如果随机变量X满足Prob(X=1)=p且Prob(X=0)=1-p，那么X就腹产参数为p∈[0,1]的伯努利分布。我们把结果1看作成功，把结果0看作失败，并记作X~Bern(p)。X也可以称为二元标示随机变量。
* 如果X~Bern(p)，那么X的均值=p，方差等于p(1-p)

## 二项分布
![bin](./pictures/bin.png)

* 一个服从Bin(n,p)的随机变量与n个相互独立且服从Bern(p)的随机变量是一样的。
* 如果X~Bin(p)，那么X的均值=np，方差等于np(1-p)

## 多项分布
* 二项分布：进行多次独立重复试验，且每次试验都有两种可能结果，某一种结果出现次数的分布
* 多项分布：进行多次独立重复试验，并且每种结果在任意一次试验中发生的概率都相等，每次试验有两种以上的可能结果，多种结果出现的某种组合的次数的分布

![multionmial](./pictures/multionmial.png)
* 如果构造随机变量Xi，表示结果i出现的次数，就得到了二项分布(出现与不出现两种情况)，Xi ~ Bin(n, pi)

## 几何分布
* 不断重复服从伯努利分布的试验，直到首次成功，则试验次数X服从几何分布

![geom](./pictures/geom.png)

## 泊松分布
* 泊松分布可以被定义为参数为n和p的二项分布的极限，其中n->∞且np->λ
