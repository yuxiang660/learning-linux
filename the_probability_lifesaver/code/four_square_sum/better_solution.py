...
def get_four_square_num(test_num):
    ...
    for a in range(int(math.sqrt(test_num)) + 1):
        for b in range(min(a, int(math.sqrt(test_num - a**2))) + 1):
            for c in range(min(b, int(math.sqrt(test_num - a**2 - b**2))) + 1):
                for d in range(min(c, int(math.sqrt(test_num - a**2 - b**2 - c**2))) + 1):
